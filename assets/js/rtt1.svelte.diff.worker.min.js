<script>
  ...
  function computeDiagnostics(diffs) {
    const d = { adds: 0, removes: 0, changes: 0 };
    for (const x of diffs) {
      if (x.type === "add") d.adds++;
      else if (x.type === "remove") d.removes++;
      else if (x.type === "change") d.changes++;
    }
    diagnostics = d;
  }
...
<div class="rtt-shell">
  <header class="rtt-shell__header">
    <div class="rtt-shell__title">RTT/1 Svelte Diff Bundle</div>
  ...
  <footer class="rtt-shell__footer">
    TriadicFrameworks · RTT/1 Svelte Diff Bundle Edition
  </footer>
</div>
